...
export function ls(flags: string[], args: string[]): string {
    ...
    try {
        ...
        return entries
            .map((file) => {
                if (!list) {
                    return `<span style="color: ${file.isDirectory ? "#50fa7b" : "#fff"}">${
                        sanitize(file.name)
                    }</span>`
                }
                ...
                const time = `${padLeft(`${file.created.getHours()}`, 2, "0")}:${
                    padLeft(`${file.created.getMinutes()}`, 2, "0")
                }`

                return `${mode} ${file.nlink} ${user} ${user} ${size} ${month} ${day} ${time} <span style="color: ${
                    file.isDirectory ? "#50fa7b" : "#fff"
                }">${sanitize(file.name)}</span>`
            })
            .join(list ? "\n" : " ")
    } catch {
...
export function cd(_flags: string[], args: string[]): string {
    if (args.length != 1) {
...
export function cat(_flags: string[], args: string[]): string {
    if (args.length < 1) {
...
export function rm(_flags: string[], args: string[]): string {
    try {
...
export function mdcat(_flags: string[], args: string[]): string {
    if (args.length < 1) {
